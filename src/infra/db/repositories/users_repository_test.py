...
)  # Só retirar este decorador quando for preciso testar o insert
...
""".format(
    ...
    # Comparando a busca com a inserção
    assert registry.first_name == mocked_first_name
    assert registry.last_name == mocked_last_name
    assert registry.age_name == mocked_age

    # Deletar o registro após inserção para preservar o banco de dados
    connection.execute(
        text(
            f"""
...
""".format(
    ...
    assert response[0].first_name == mocked_last_name
